...
module.exports = {
  ...
  postAction() {
    ...
    return co(function* () {
      let flag = yield servicePackageOrderUtil.validmakeAppointmentOrder(servicePackageOrderId, userId, workTime);
      ...
      if (flag) {
        ...
        let dt = yield makeAppointmentOrderService
          .createOrder(userId, servicePackageOrderId, workTime, address, that.req, items, instructions, checklist_imgs, user.userName);
        resultObj = {
          code: '200',
          msg: '',
          data: {}
        ...
        let _account = yield TransactionMysqlService.getAccountByUserIdAndDoctorId(userId + '');
        resultObj.data.amount = Math.floor(_account.amount * 100) / 100;
        console.log('创建订单的返回值', result);
        // 如果是金牌会员产后复查，发送短信通知
        yield Backend.service('applet', 'send_msg').makeAnAppointmentNotice(servicePackageOrderId, userId, workTime, items);
