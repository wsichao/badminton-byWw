...
let red_service = Backend.service('1/red_paper','red_paper');
...
module.exports = {
  ...
  getAction: function () {
    ...
    let moment_ids = [];
    let moment_map = {};
    let moment_rest_num_map = null;
    let config_id = '5954af71b1bce56941cdeb7f';
    return config_service.getHeavenConfigInfo()
      .then(function (_config) {
        resObj.isActive = _config.isActive;
        if(resObj.isActive ){
          return user_service.getRestTimesToday(userId, _config.timesPerPeoplePerDay);
        }
      })
      .then(function (_times) {
        //console.log('_times:', _times);
        resObj.chanceNumber = _times || 0;
      ...
      .then(function (_users) {
        ...
        moment_ids = _.map(users, function(user){
          return user.momentRef + '';
        })
        //获取每个动态的红包剩余数
        //console.log('moment_ids:', moment_ids);
        return red_service.getRedPaperCount(moment_ids, userId);
      })
    .then(function(_res) {
      //console.log('moment_rest_num_map:', _res);
      moment_rest_num_map = _res;
      //获取动态信息
      return moment_service.getMomentsDeepInfoByIds(moment_ids , '', null, userId);
    })
    .then(function(_moments){
      moment_map = _.indexBy(_moments || [], '_id');
      ...
      .then(function (_shops) {
        ...
        resObj.items = users.map(function (item) {
          ...
          let avatar = isShopAuth ? (item.shopAvatar || '') : (item.avatar || '');
          ...
          let obj = {
            ...
            currentMoment: item.currentMoment && item.currentMoment.substr(0, 200) || (picMoment || ''),
            remainMemberSize: isShopAuth ? remainMemberSize : 0,
            momentInfo: moment_map[item.momentRef + ''] || null
          }
          ...
          obj.redPaperNumber = moment_rest_num_map[item.momentRef + ''] || -1;
          obj.hongbaoNumber = moment_map[item.momentRef + ''].hongbaoRestCount || -1;
          delete moment_map[item.momentRef + ''].hongbaoRestCount;
          return obj;
        });
