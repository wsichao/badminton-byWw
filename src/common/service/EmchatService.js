...
var token;
var client_id = ZLYCARE_DOC_EMCHAT_ID_RUNTIME;
var client_secret = ZLYCARE_DOC_EMCHAT_SECRET_RUNTIME;
...
var http_request = function (json) {
  ...
  Promise.resolve().then(function () {
    "use strict";
    if (token === undefined) {
      return BizHelper.util.cfcall(getToken);
    } else {
      return false
    }
...
/*
 ...
 json.callback: 回调函数
 */
var _http_request = function (json) {
  var json = json || {};
  json.data = json.data || {};
  ...
  var path = '/' + ZLYCARE_DOC_EMCHAT_ORG_NAME + '/' + ZLYCARE_DOC_EMCHAT_APP_NAME + json.path;
  //path = '/zly/zlycare-doc-internal' + json.path;
  //请求参数
  var options = {
    host: 'a1.easemob.com',
    path: path,
...
function createUser(username, password, nickname) {
  var deferred = Backend.Deferred.defer();
...
function deleteFriend(username, friendname) {
  var defer = Backend.Deferred.defer();
  http_request({
    path: '/users/' + username + '/contacts/users/' + friendname,
    method: 'DELETE',
    callback: function (data) {
...
//往黑名单中加人
function addUserForBlacklist(username, users) {
  var defer = Backend.Deferred.defer();
...
//从黑名单中减人
function deleteUserFromBlacklist(username, blackuser) {
  var defer = Backend.Deferred.defer();
  http_request({
...
function getChatRecord(json) {
  http_request({
    path: '/chatmessages/' + json.timeStr,
    method: 'GET',
    callback: function (data) {
      console.log(data);
      typeof json.callback == 'function' && json.callback(data);
...
// 获取聊天记录文件
/**
 * json.time : 聊天记录查询时间 示例:2017060518
 * json.callback : 回调函数
 *
 */
function getChatFile(json) {
  "use strict";
  http_request({
    path: '/chatmessages/' + json.time,
    method: 'GET',
    callback: function (data) {
      typeof json.callback == 'function' && json.callback(data);
    }
