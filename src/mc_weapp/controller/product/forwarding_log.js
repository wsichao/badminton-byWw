...
module.exports = {
  ...
  async update(share_user_id, scene_id, product_id) {
    ...
    Backend.service("mc_weapp", "user_activity_log").shareLog(share_user_id, scene_id, product_id);
  },
  async getAction() {
    try {
      const share_user_id = this.query.user_id;
      const user_id = await this.getUserId(this.query.recommend_user_id);
      const product_id = this.query.product_id;
      const scene_id = await this.getScenceId(user_id);
      await this.update(share_user_id, scene_id, product_id);
      return this.success({
        code: "200",
        msg: ""
      })
