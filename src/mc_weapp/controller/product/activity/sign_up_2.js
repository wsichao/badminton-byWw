...
module.exports = {
  __rule: function (valid) {
    return valid.object({
      ...
      school_info: valid.string().required(),
      student_name: valid.string().required(),
      build_leader: valid.string().empty(""),
      declaration: valid.string().required(),
      imgs: valid.array().required()
    });
  },
  async check() {
  ...
  async save() {
    ...
    const school_info = post.school_info;
    const student_name = post.student_name;
    const build_leader = post.build_leader;
    const imgs = post.imgs;
    const declaration = post.declaration;
    const data = {
      school_info,
      student_name,
      build_leader,
